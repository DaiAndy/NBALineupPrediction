...
class nbaPredictor(nn.Module):
    ...
    def forward(self, x, home_team):
        ...
        # Mask out players not in that home_team
        # Note: The keys in teamArray are still the original strings (e.g. "SAS")
        # but 'home_team' is an integer from team_encode now.
        # If we want to use numeric IDs as the key, we'd have to build teamArray differently.
        # For now, we'll skip the mask logic or do an inverse transform:
        homeFilter = torch.full_like(out, -1e9)
        for i in range(x.size(0)):
            # Convert numeric back to string
            team_id = home_team[i].item()
            team_str = team_encode.inverse_transform([team_id])[0]

            if team_str not in teamArray:
                # If you see this, that means 'team_str' wasn't in the test data from earlier.
                # So it's possible your train data has extra teams not in df_test, etc.
                print(f"{team_str} is not in the Team Array")
                continue

            # The set of possible players for that team
            possible_players = teamArray[team_str]
            # But possible_players is a set of original player names (strings).
            # Our network now references players by integer IDs.
            # So you need to convert those strings to their label-encoded IDs to do the mask.
            possible_player_ids = [player_encode.transform([p])[0] for p in possible_players if p in player_encode.classes_]

            # Fill them with 0
            for pid in possible_player_ids:
                homeFilter[i, pid] = 0.0

        return out + homeFilter
